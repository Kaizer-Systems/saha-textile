...
Customer messaging flows through `NotificationPort`, with SMS, WhatsApp, and email behind swappable adapters. The locked primary provider is **MSG91** (SMS / WhatsApp / Email, including auth OTP), with an optional email fallback (`resend`/`ses`/`smtp`) that is never primary. The API validates the provider configuration and currently binds a `ConsoleNotificationAdapter` development seam used by the live OTP orchestration. Mongo models/repositories now persist notification settings, templates, usage/outbox facts, consent events and broad audit evidence. A real MSG91 adapter, fallback adapters and complete notification workflow/side-effect orchestration remain planned, so provider delivery is not production-implemented. Transactional and marketing controls are separate.
