...
- **SCAFFOLDED:** the deterministic source-only MongoDB catalogue renders all 34 implemented Mongoose models, 424 fields, 94 indexes, 27 temporary-shape warnings, and sanitized synthetic examples that omit all 13 excluded-by-default fields. Schema Nebula separately publishes 64 nodes / 31 graph-backed current / 33 target; three current catalogue collections remain outside that ratified graph. Stable target schemas, mappings, migrations, and target-only collections remain promotion gates.
