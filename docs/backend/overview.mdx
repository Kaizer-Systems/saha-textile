...
| Mongo adapter            | Seven models, repositories, mappers, indexes, seed tool, `MongoTransactionManager`, self-hosted Docker `rs0` profile                                                       | Session-aware workflow adoption, stronger nested validation, migrations, backups/restore             |
| Tests                    | Contract/core suites, 28 API platform/CSRF tests, gated repository plus six rs0 transaction tests                                                                          | Complete auth/ownership/idempotency/workflow coverage and CI execution of every DB-gated proof       |
| Generated references     | API `/openapi.json`; portal OpenAPI + Scalar scaffold; seven-model source catalogue                                                                                        | Complete OpenAPI operation metadata and target-complete database catalogue                           |
