...
| Authentication           | Verified signup, email/phone password and OTP login, provider verifiers and identity links, sessions/password/account management, guest-cart adoption, admin PIN/invites/resume, 89-code registry, BOLA | Provider activation/browser proof, PIN browser proof, pending-intent continuation, granular migration, and provider-backed delivery |
...
| Generated references     | API `/openapi.json`; 95-path/111-operation Scalar scaffold; 40-model source catalogue                                                                                                                   | Complete OpenAPI operation metadata and target-complete database catalogue                                                          |
...
3. Order creation commits order save and cart consumption in one `TransactionManagerPort` unit of work. Idempotency, inventory, payment, audit, and outbox participation remain open.
4. Browser authentication is cookie-only with session-family rotation, reuse revocation, session-bound CSRF, and audience separation. Provider activation and supported-browser evidence remain separate readiness gates.
