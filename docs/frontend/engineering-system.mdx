...
The auth interceptor sends `withCredentials: true` on every request so API-issued session cookies flow — the locked target is httpOnly cookie sessions plus CSRF (Chunk D). A `localStorage` bearer token is a **transitional** fallback that is removed when the cookie-session refactor lands.
