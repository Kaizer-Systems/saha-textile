...
`packages/contracts/src` contains zod-backed common locale/money/pagination/error primitives and families for catalogue placements/attributes/options/variants/bundles/relations, media, inventory, content, cart, order, currency, promotion, shipping, users, auth/session/admin auth, permissions, roles, consent, audit, and notifications. The active locale set is runtime configuration, not a contract enum. Most expanded families describe validated boundaries but are not yet wired into Angular workflows or current API operations.
