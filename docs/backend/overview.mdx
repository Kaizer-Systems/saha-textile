...
| HTTP modules             | Health, auth, privacy, catalogue, currency, promotions, cart, orders                                                                                                                                                                                                                  | Complete public/admin route families, DTO families, versioning and operation metadata                                              |
| Authentication           | Cookie-only access/opaque-refresh sessions, sid-bound logout revocation, session-bound CSRF (Policy B), audiences, storefront password/OTP/reset/email verification, admin recovery/PIN/invites/HTTP resume, permission-version checks, cart/`st_guest` ownership and order ownership | OAuth, idle-lock client, first-admin bootstrap, fine-grained assignment enforcement, guest→user merge and provider-backed delivery |
| Contracts                | Zod schemas for common/auth/permission/role/catalog/media/inventory/content/commerce families; 154 tests                                                                                                                                                                              | Runtime wiring and complete operation-level OpenAPI mapping                                                                        |
| Core domain              | Repository/provider ports, auth rate-limit policy, role repository, pure pricing/visibility helpers, mechanically enforced type-only contract boundary; 87 tests                                                                                                                      | Use-case/state-machine adoption and remaining policy coverage                                                                      |
| Mongo adapter            | 34 models with explicitly declared physical collection names, 100 opt-in adapter tests, mappers, indexes, seed tool, `MongoTransactionManager`, self-hosted Docker `rs0` profile                                                                                                      | Role-assignment API adoption, remaining HTTP/workflow adoption, stronger nested validation, migrations, backups/restore            |
| Tests                    | Contract/core suites, 97 API tests and 100 opt-in Mongo adapter tests across platform, auth, RBAC, transactions and repositories                                                                                                                                                      | Full live HTTP matrix/idempotency/workflow coverage and CI execution of every DB-gated proof                                       |
| Generated references     | API `/openapi.json`; 45-path/49-operation Scalar scaffold; 34-model source catalogue                                                                                                                                                                                                  | Complete OpenAPI operation metadata and target-complete database catalogue                                                         |
...
10. API has 97 tests. All 100 Mongo adapter tests remain explicitly gated by `RUN_DB_IT=1`; the transaction and repository suites have been proven against rs0 but must remain part of CI readiness work.
