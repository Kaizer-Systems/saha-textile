...
Gate questions and blast radii live in `docs/_data/instruments/decision-gates.json`; their live state comes from the marked Portal truth snapshot in `project-progress.md`. The build-time compiler cross-checks that state against the roadmap gate table and resolved owner-log decisions before publishing the typed dataset to this console.
