...
| Mongo adapter            | 32 models with explicitly declared physical collection names, 89 opt-in adapter tests, mappers, indexes, seed tool, `MongoTransactionManager`, self-hosted Docker `rs0` profile                                               | Remaining HTTP/workflow adoption, stronger nested validation, migrations, backups/restore                          |
| Tests                    | Contract/core suites, 64 API unit/guard tests and 89 opt-in Mongo adapter tests across baseline, transactions, auth, catalogue, inventory/media, governance/content, order+cart tx and collection-name alignment              | Full live HTTP matrix/idempotency/workflow coverage and CI execution of every DB-gated proof                       |
...
10. API has 64 unit/guard tests. All 89 Mongo adapter tests remain explicitly gated by `RUN_DB_IT=1`; the transaction and repository suites have been proven against rs0 but must remain part of CI readiness work.
