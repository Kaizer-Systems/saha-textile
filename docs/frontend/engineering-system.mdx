...
The auth interceptor sends `withCredentials: true` on every request so the implemented httpOnly cookie sessions flow. It still attaches a legacy `localStorage` bearer token, but the API's `SessionGuard` deliberately ignores `Authorization`; this is stale frontend migration debt, not a supported authentication fallback.
