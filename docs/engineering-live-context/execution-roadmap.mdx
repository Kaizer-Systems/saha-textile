...
- **SCAFFOLDED:** the source-only MongoDB catalogue renders all 32 implemented Mongoose models, 406 fields, 89 indexes, 27 temporary-shape warnings, and sanitized synthetic examples that omit all 13 excluded-by-default fields. Only `reviews` among the D/E additions resolves to an exact locked Schema Nebula name; lowercase multiword collection defaults do not promote mismatched stars. Stable target schemas, mappings, migrations, and target-only collections remain promotion gates.
