...
import { Component, HostListener, PLATFORM_ID, TemplateRef, inject, viewChild } from '@angular/core';
...
export class ExitModal {
	...
	exit$: Observable<boolean> = toObservable(this.siteConfigStore.exit) as Observable<boolean>;
	...
	onMouseOut(event: MouseEvent) {
		if (event.clientY <= 0) {
			if (this.exit === true) {
				void this.openModal();
				this.siteConfigStore.updateSession('exit', false);
			}
		}
	}
