---
title: 'Authentication Architecture, Data Model, and Request Plan'
description: 'Canonical target authentication boundary, persistence model, and secure request plan.'
audience: [backend, security, agent]
document_role: canonical
last_verified: '2026-08-13'
search_keywords: 'authentication authorization cookies csrf oauth otp pin sessions database'
---

# Authentication Architecture, Data Model, and Request Plan

:::caution Superseded identity collections
**`DEC-ACCOUNT-SEPARATION` (2026-08-12)** amends the single-`users` collection model described below. Live identity is split into `customers` (shoppers, `cus_` ids) and `adminUsers` (operators, `adm_` ids). Treat §7.1 `users` and any single-population `UserRepository` language in this document as historical target text superseded by that lock, the owner decisions log, and `account-separation-task-handoff.mdx`. Session, cookie, CSRF, OTP, and OAuth mechanics in later sections remain the architecture contract unless a newer lock amends them.
:::

**Status:** Target architecture and implementation contract
**Last reconciled:** 2026-07-25 (identity collections amended 2026-08-12)
...
lives in `project-progress.mdx`; actual request/response shapes live in
...
The canonical variable inventory is `environment-variables.mdx`. Auth-relevant
groups include:
...
- Current verification status is recorded in `project-progress.mdx`.

## 13. Open decisions

There is no question register in this architecture file. Any unresolved
auth-adjacent owner choice belongs in `pending-decisions.mdx` under a stable
`DEC-*` identifier.
