...
| ------------------------ | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Fastify/NestJS bootstrap | Helmet, global rate limit, CORS, OpenAPI, shutdown hooks       | Trusted proxy chain, real client IP, route-specific limits, secure CSP, sanitized global errors         |
...
| Contracts                | Zod schemas for core catalogue/commerce shapes                 | Separate request/update/response schemas, sensitive-field policies, complete OpenAPI mapping            |
| Core domain              | Repository/provider ports and pure pricing helpers             | Independent domain entities/use cases, unit of work, notification/media ports, complete policy coverage |
| Mongo adapter            | Seven models, repositories, mappers, indexes, seed tool        | Replica-set parity, transactional context, stronger nested validation, migrations, backups/restore      |
...
9. The Mongo connection/env examples still contain hosted-cluster assumptions that conflict with the self-hosted Docker MongoDB lock.
