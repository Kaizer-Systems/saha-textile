...
- **DONE:** canonical Engineering Live Context, build-indexed LIVE/search surfaces, one Angular Storybook, scoped TypeDoc, shared theme contract, and the atomic persistent local composite runner.
- **BLOCKED:** Scalar and mandatory Test Request wait for deterministic complete OpenAPI, security/interaction semantics, drift checks, and an approved development/staging request target.
- **BLOCKED:** the generated MongoDB field catalogue waits for stable models, nested validators/indexes, mappings, migrations, and sanitized deterministic examples.
