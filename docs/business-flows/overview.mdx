...
wide: true
description: Interactive map connecting current application behavior, locked target flows, and failure recovery.
status: scaffolded
...
- **Locked target** — the authoritative intended behavior.
...
| What must the business flow eventually do? | Owner decisions and reconciled technical KB                    |
...
The portal may document a locked target before code exists, but the lifecycle status and **Current code** lens must make that distinction unmistakable.
