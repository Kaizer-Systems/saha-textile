...
current Storybook and TypeDoc children, and future Scalar/OpenAPI and
database-catalogue assets to the browser. Protecting only HTML routes or hiding
navigation would still expose the underlying static assets. The public
repository also means deployment controls cannot turn committed secrets into
private information.
