...
The Mongo adapter contains real connection configuration, 34 models, indexes, mappers, repositories, seed tooling, 100 gated integration tests, and a bound transaction manager. The adapter remains partial at the whole-roadmap level. The interactive [Schema Nebula](./schema-nebula) maps the governed 64-collection target with 31 current nodes, while the separate source-only catalogue documents all 34 current Mongoose models.

## Current implementation boundary

Models currently exist for the original commerce/catalogue surface plus auth sessions/challenges/tokens/rate limits, roles/user assignments, consent, catalogue placement/facets/attributes, variants/bundles/relations, media, inventory ledgers/cost layers, audit/notification/outbox data, FAQs, product questions, reviews and rating aggregates. Chunk E persistence is complete and its atomic workflows are rs0-proven; the new RBAC repositories are persistence capability, not current API assignment enforcement. Model presence still does not prove every HTTP use case has adopted those repositories or prove production migration, backup, restore and operational readiness.
...
The catalogue generator imports model metadata without opening a database connection. It remains explicitly current-model-only and scaffolded. The deterministic 2026-08-09 regeneration measures **34 models / 424 fields / 94 indexes / 27 temporary shapes** and omits all **13** excluded-by-default fields from synthetic previews.
...
- Every model declares its physical collection name. Schema Nebula promotes 31 graph-backed names; `authRateLimits`, `productQuestions`, and `ratingAggregates` remain current catalogue evidence outside the fixed graph.
- DB integration suites remain opt-in through `RUN_DB_IT=1`; all 100 adapter tests have recorded rs0 proof, while CI and HTTP/workflow adoption remain separate readiness requirements.
