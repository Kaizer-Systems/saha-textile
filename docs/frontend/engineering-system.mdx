...
`packages/contracts/src` contains zod-backed common locale/money/pagination/error primitives and families for catalogue placements/attributes/options/variants/bundles/relations, media, inventory, content, cart, order, currency, promotion, shipping, customer, admin-user, auth/session/admin auth, customer and admin-user authority, permissions, roles, consent, audit, and notifications. The active locale set is runtime configuration, not a contract enum. The 103-code permission registry and admin authority contracts now back API role/permission/user-authority routes and `GET /auth/admin/me` effective permissions. Most other expanded families describe validated boundaries without Angular workflow or current API-operation adoption.
