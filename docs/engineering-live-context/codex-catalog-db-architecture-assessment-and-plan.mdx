---
title: 'Catalog and Commerce Database Architecture'
description: 'Canonical catalog, category, product, search, commerce, and reporting data architecture.'
audience: [backend, data, agent]
document_role: canonical
last_verified: 2026-07-31
search_keywords: 'catalog category product variants search commerce reporting mongodb schema'
---
...
**Authority:** `owner-decisions-log.mdx`; unresolved choices: `pending-decisions.mdx`

This is a target design, not proof that every collection or capability exists.
Current delivery status lives in `project-progress.mdx`; Zod contracts, adapter
models/indexes, tests, and runtime behavior govern implemented truth.

## 1. Sources Read

- Current `owner-decisions-log.mdx`, `pending-decisions.mdx`, and retained technical KB.
...
- ~~`ProductStatus` is only `draft | published | archived`~~ — **CLOSED 2026-07-31.** The owner-locked lifecycle is exactly `draft | live | disabled | discontinued` (see `owner-decisions-log.mdx`), now implemented in `packages/contracts/src/product.ts` with a `lifecycle` timestamp block, enforced in the Mongo product repository through a `CatalogAudience`, and proved by replica-set integration tests. Every `draft | published | hidden | archived | discontinued` sketch below predates that lock and is superseded: `published` → `live`, `hidden`/`archived` → `disabled`, and `discontinued` keeps its meaning plus the 30-day rich-data purge to a tombstone.
...
> **Status field superseded (2026-07-31).** The `status` and `lifecycle` keys in the shape below predate the owner lock. The implemented and authoritative values are `draft | live | disabled | discontinued` with a `lifecycle` block of `liveAt`, `disabledAt`, `discontinuedAt`, `richDataPurgedAt`, and `statusReason`. Only `live` is storefront-queryable. The rest of this shape remains target guidance.
...
- `QuestionAnswer` (customer Q&A is specified in this document and
  `owner-decisions-log.mdx`; it is separate from editorial FAQ)
...
1. Review the applicable `DEC-*` gates in `pending-decisions.mdx`; proceed with stable seams and block only irreversible policy choices.
...
22. Add separate editorial FAQ and customer Q&A modules per this document and
    `owner-decisions-log.mdx`, reusing editor/table primitives without merging
...
Resolved decisions are recorded once in `owner-decisions-log.mdx`; they are not
...
All unresolved owner choices live in `pending-decisions.mdx`. Catalog and
