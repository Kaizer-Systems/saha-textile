...
4. Link to the cart Zustand store documentation.
...
→ Next.js route
→ React component
→ Zustand/TanStack Query operation
→ API endpoint
→ MongoDB collection
...
- TanStack Query keys and invalidation rules.
- Zustand stores.
...
Then use the **same Caddy reverse proxy already running on the DigitalOcean Droplet** to serve those static files.
...
- Runtime resource usage is essentially limited to Caddy serving static files and the storage occupied by the generated assets.

Caddy provides static-file serving through its `file_server` directive. ([caddyserver.com](https://caddyserver.com/docs/caddyfile/directives/file_server?utm_source=chatgpt.com))

---

# Private authentication

For one developer, use Caddy’s built-in `basic_auth` protection over HTTPS.
...
Caddy’s `basic_auth` directive protects files and directories using usernames and hashed passwords; Caddy explicitly warns that it must not be used over plain HTTP, so keep the docs endpoint HTTPS-only. ([caddyserver.com](https://caddyserver.com/docs/caddyfile/directives/basic_auth?utm_source=chatgpt.com))
...
- **Private access:** Caddy Basic Auth over HTTPS.
- **Hosting:** Static output served by the existing Caddy container on the existing Droplet.
