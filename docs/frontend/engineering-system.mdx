...
`packages/contracts/src` contains zod-backed common locale/money/pagination/error primitives and families for catalogue placements/attributes/options/variants/bundles/relations, media, inventory, content, cart, order, currency, promotion, shipping, customer, admin-user, signup, OAuth, storefront account, auth/session/admin auth, authority, permissions, roles, consent, audit, and notifications. The active locale set is runtime configuration, not a contract enum. The 89-code permission registry and admin authority contracts back API authority routes and `GET /auth/admin/me` effective permissions. Most other expanded families describe validated boundaries without Angular workflow or current API-operation adoption.
