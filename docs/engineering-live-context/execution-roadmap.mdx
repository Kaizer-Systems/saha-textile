...
- **SCAFFOLDED:** the source-only MongoDB catalogue renders all 14 implemented Mongoose models, 192 fields, 43 indexes, 13 temporary-shape warnings, and sanitized synthetic examples that omit all 12 excluded-by-default hash fields. D1’s lowercase physical auth collection names do not promote the mismatched Schema Nebula targets; stable target schemas, mappings, migrations, and target-only collections remain promotion gates.
