...
- **SCAFFOLDED:** the source-only MongoDB catalogue renders the seven implemented models, fields, indexes, temporary-shape warnings, and sanitized synthetic examples. Stable target schemas, mappings, migrations, and target-only collections remain promotion gates.
