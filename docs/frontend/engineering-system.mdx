...
`packages/contracts/src` contains zod-backed contract modules for cart, category, common pagination/problem shapes, currency, order, product, promotion, shipping, and user, plus scaffolded auth/session/admin-auth/consent/audit/notification families. The latter describe locked boundaries but are not yet wired into the Angular applications or current API lifecycle.
