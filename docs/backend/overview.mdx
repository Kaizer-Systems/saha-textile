...
| HTTP modules             | Health, auth, admin authorization, privacy, catalogue, currency, promotions, cart, orders                                                                                                                                | Complete public/admin route families, DTO families, versioning and operation metadata                        |
| Authentication           | Cookie-only sessions, sid-bound logout, session CSRF, audiences, storefront auth, admin recovery/PIN/invites/resume, 33-code registry, assignment resolution, deny-by-default admin management, operator bootstrap, BOLA | OAuth, Security Settings/idle-lock clients, browser PIN proof, guest→user merge and provider-backed delivery |
...
| Mongo adapter            | 34 models with explicitly declared physical collection names, 103 tests total (99 rs0-gated), mappers, indexes, seed tool, `MongoTransactionManager`, self-hosted Docker `rs0` profile                                   | Remaining HTTP/workflow adoption, stronger nested validation, migrations, backups/restore                    |
...
| Generated references     | API `/openapi.json`; 52-path/59-operation Scalar scaffold; 34-model source catalogue                                                                                                                                     | Complete OpenAPI operation metadata and target-complete database catalogue                                   |
