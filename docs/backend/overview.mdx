...
Pass 4 changed documentation and portal interaction only. Later portal tooling now source-generates a scaffolded OpenAPI/Scalar surface and current-model catalogue; it still does not replace auth, create Mongo transactions, install Meilisearch, wire providers, or complete either generated contract.
